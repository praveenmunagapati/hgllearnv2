            ...
            {clientDetails?.slice(StartValue, EndValue)?.map((val, i) => {
              return (
                <div key={i} className="flex w-full relative">
                  <div
                    className="w-20  md:w-12 lg:w-12 xl:w-20 xxl:w-20 absolute left-8 top-5 border-4
                 border-white rounded-full "
                  ...
                  <div className="  h-32 w-9/12 mx-auto rounded-2xl px-16 py-3 text-center bg-slate-200 ">
                    ...
                    <div className="text-xs h-16 text-gray-600 pb-4 w-full overflow-scroll info hidden ">
                      {val.description}
                    </div>
                  </div>
                </div>
